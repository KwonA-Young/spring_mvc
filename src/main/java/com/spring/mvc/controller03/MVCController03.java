...
public class MVCController03 {
    ...
    public String naver() {
        ...
        // 이 과정에서 포워딩과는 달리 model.addAttribute()처럼 자료를 첨부해 이동시킬 수는 없다.
        // 하지만 get 방식으로 이동하는 만큼 쿼리스트링을 붙여서 보내는 것은 가능하다.
        return "redirect:https://www.naver.com";
    }

    @RequestMapping("/goFoodform")
    public String goFoodform(Model model) {
        // 리다이렉션은 model.Attribute()를 통한 데이터 전송이 불가능하다.
        model.addAttribute("title", "음식주문 키오스크"); // title 라는 이름으로 "음식주문 키오스크" 문자열 적재.
            // http://localhost:8181/chap03/foodform 이라는 주소로 이동(리다이렉트 이므로 파일지정이 아님.)
